...
public class ArticleServiceImpl implements ArticleService {
  ...
  public List<ArticleDTO> getAll() {
    return articleRepository.findAll().stream()
        .map(article -> new ArticleDTO(article, false, false, false)).collect(Collectors.toList());
  }
  ...
  public ArticleDTO add(ArticleDTO articleDTO, Long userId) {
    ...
    ArticleStatusHistoryDTO articleStatusHistoryDTO =
        new ArticleStatusHistoryDTO(article, true, true);
  ...
  public void createStatusHistory(ArticleStatusHistoryDTO articleStatusHistoryDTO) {
    ArticleStatusHistory articleStatusHistory =
        new ArticleStatusHistory(articleStatusHistoryDTO, true, true);
      ...
      case DRAFT:
        switch (articleDTO.getStatus()) {
          ...
          case PUBLISHED, REJECTED:
            throw new IllegalStatusException(articleStatus.toString(),
                articleDTO.getStatus().toString());
        }
        break;
      case UNDER_REVIEW:
        switch (articleDTO.getStatus()) {
  ...
  public List<ArticleDTO> getAllByUser(Long userId) {
    return userRepository.findById(userId).get().getArticleList().stream()
        .map(article -> new ArticleDTO(article, false, false, false)).collect(Collectors.toList());
  }
